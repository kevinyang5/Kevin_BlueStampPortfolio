...
My final milestone was to complete my final product for my sprint timer before any possible further modifications. This milestone consisted of writing code to make the actual timer, cadding cases for both setups to store all the hardware and other parts, wiring and soldering the metal pushbuttons to both the starting and finishing setups, and assembling everything. With the code, I allowed the ESP32 to start timing using its internal clock when an object was detected by the VL53L1X motion sensor. The VL53L1X always records the distance of any object that is withing 4 meters of the sensor. The code ensure the VL53L1X only starts the ESP's internal clock, or acts like it detected an object, if any object crosses a specific threshold of distance from the sensor because my code only allowed the VL53L1X to detect an object if it crossed the threshold. Even if there were another object within 4 meters, the code made it so the VL53L1X acts like it didn't detect an object unless it was within the threshold. (not done yet)
### Metal Pushbuttons
Both of the starting and finishing setups have a metal pushbutton wired to it. The pushbuttons are used to reset either the starting or finishing setup. I used the "Metal Pushbutton with Wires - Momentary (16mm, Red)" by SparkFun. These pushbuttons have five wires: red, green, blue, black, and white. I only needed them to have a button which can reset any of the setups, so I only needed to use four of those wires. The blue (Normally Closed wire or NC1) wire isn't needed when the pushbutton is only used to reset the setup. The red (+) wire connects to the positive power supply and provides a power path, and it's connected to the D13 pin on the ESP32 because when the pushbutton is idle, it keeps the GPIO pin at HIGH, and when it's pressed, it pulls the GPIO pin to low, which pulls the voltage down. The white (Normally Open or NO1) wire is always open unless the button is pushed, which only allows current to flow when the button is pressed. The NO1 wire is connected to the EN (RESET) pin on the ESP32, and this allows the ESP32 to reset because when the button is pressed, it allows current to flow which triggers the reset. The green (Common or C1) wire acts as the main connection for the power source and the starting point for the electrical circuit, and will direct current to either the normally open or closed terminals. The C1 wire is connected to GND (Ground) to allow a path for the current to flow from a power source. The black (-) wire also connects to GND because it provides the return path for any electrical current which completes the circuit. These four wires allow me to reset the ESP32 and thus the whole setup with just the push of a button. 

## Challenges - Final Milestone
I only really ran into one major challenge throughout this milestone. When I ran some trials by just waving my hand over the motion sensors, the elapsed time was always inaccurate. Instead of resetting after each trial, the elapsed times kept adding up after each test. At first, I thought it was an issue with my code, but when going through my code with my instructor, we realized that the problem was due to offset between the internal clocks of both ESPs, which caused the inaccurate time recordings. When starting a new time trial, I would reset both ESP32s one at a time, which started the internal clocks at different times, resulting in some offset. To fix this, I had to change the code of both setups to only ensure the use one internal clock, so I wouldn't have to reset both setups at the same time to successfully run an accurate time trial. The plan was to only start the internal clock of the starting ESP when the starting motion sensor detected an object within the threshold. When motion was detected, the starting ESP would send a signal to the finishing ESP, where it would tell the finishing motion sensor to get ready to detect an object. Once the finishing motion sensor detected an object, it would send that time back to the starting ESP where it would display the final elapsed time on the LCD screen. This prevented any error due to offset because the original code had the finishing setup calculate the elapsed time and display it on its LCD screen, which made it necessary to reset the finishing setup before every trial, and that would result in error if both setups weren't reset at the same time. With this new code, I would only need to reset the starting setup before each trial. The only thing I really needed to do to change the code was to move some lines of code to its opposite setup's code, and that didn't give me any trouble at all. This challenge and what I did to get through it continues to show the importance of carefully going through code and adding comments to it to ensure you know the function of every line so its easy to make changes. 
